...
****************************************************************************************
*                                  私聊
* 私聊信息的格式 @xyq:somethingNeedYouKnow
* @Desc  : 主要实现：
			输出好友列表给客户端
			获取聊天对象返回判断结果给客户端
			查询聊天记录
			处理记录发送给客户端
			接收发送私聊消息,判断对方状态后进行传递
* @return: 无返回值
****************************************************************************************
*/
...
****************************************************************************************
*                                  响应群聊
* @群聊信息的格式  : $xyq:somethingNeedYouKnow
* @Desc  : 主要实现
			1）接收群名返回判断结果给客户端
			2）输出历史记录的六条
			3）接收消息并作转发
...
void recvFile(struct User *user)
{
    ...
    int addrlen=sizeof(struct sockaddr);
	buf = (char *)malloc(sizeof(char)*BUFFER_SIZE);
	//从未读消息中提取出所有的文件类消息，进行存储并计数， 未读文件消息格式 ~xxx:www.c
	for(i=0;i<user->unread_msg_num;i++){
		if(user->unread_msg[i][0]=='~'){
			j=1;
			while(user->unread_msg[i][j]!=':')
	...
	if(strcmp(recv_text,"0")==0){
		...
		*/
		...
		if(fp!=NULL){
			cnt = 0;
			/* 获取客户端地址 */
			recvfrom(user->udp_sockfd,buf,BUFFER_SIZE,0,(struct sockaddr *)&client,&addrlen);
			DPRINTF("[ \033[34mInfo\033[0m ] funserver.c recvFile():recvclient:%s\n",buf);
			...
			while((fileTrans = fread(buf,sizeof(char),BUFFER_SIZE,fp))>0){
				...
				sleep(1);
				//printf("udp_sockfd=%d\n",user->udp_sockfd);
				if(sendto(user->udp_sockfd,buf,fileTrans,0,(struct sockaddr *)&client,addrlen)<0)
					break;
			...
			for(i=0;i<user->unread_msg_num;i++){
				//printf("user->unread_msg[%d]=%s unread_clear_tex=%s\n",i,user->unread_msg[i],unread_clear_text);
